...
runDummy :: RunTask -> MyoE RunError (ConcNvimS Env) ()
runDummy _ =
...
runSpec = do
  let ident = Str "cmd"
  myoAddSystemCommand $ NO $ AddSystemCommandOptions ident ["ls"] (Just (Str "dummy")) Nothing Nothing
  _ <- addRunner (Str "dummy") runDummy (const True)
  lift $ myoRun (NO ident)
