...
-- import Ribosome.Config.Setting (Settings.maybe)
...
--     Right _ -> pure ()
--     Left _ -> stop RunError.SocketFailure
...
--   tmuxSocket <- Settings.maybe Settings.tmuxSocket
--   paneId <- Views.paneId paneIdent
--   runRiboTmux $ pipePane paneId ""
--   void $ tryAny $ removeFile logPath
--   pure $ state <$> findTmuxPidIO tmuxSocket paneId
--   where
--     state (Just pid) =
...
-- tmuxCheckPending _ =
--   pure (pure ExecutionState.Unknown)
...
--       Log.debug $ "running tmux system task `" <> identText commandIdent <> "` with log `" <> show logPath <> "`"
--       paneId <- Views.paneId paneIdent
--       waitForSocket logPath
--       runRiboTmux do
...
--       Log.debug $ "running tmux shell task `" <> identText commandIdent <> "`"
--       paneId <- Views.paneId paneIdent
...
--   paneIdent <- stopNote (RunError.Unsupported "tmux" "capture") (taskPane details)
--   paneId <- Views.paneId paneIdent
